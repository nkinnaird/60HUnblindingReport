...
\section{Gain Correction Procedure}
	...
	Gain correction method: Default by the Italian Calibration Team (\DB{16003}) \\
...
\label{Sec:PileupCorrection}
	...
	Pileup correction method: Asymmetric shadow window (\DB{14394} and \DB{14830}) \\
	...
	\begin{enumerate}
		...
		\item{The errors of the pileup corrected histogram were determined to be: 
			...
			\end{gather}
		where $N_{1}$ is the number of doublets where both singlets were below threshold, and $N_{4}$ is the number of doublets where both singlets were above threshold, and this is a quantity evaluated at each time bin (\DB{14830}). A histogram of error multipliers was created by factoring out the $N_{corrected}$ term, which is then applied to the bin errors before fitting. This is true even for the ratio errors to good approximation (\DB{2896} Appendix - John Paley's thesis). Note that I did not time randomize the $N_{1}$ and $N_{4}$ entries when constructing the correct errors, which is a negligble effect. This is justified in \DB{14394} slide 11, which shows that this is about a 2 ppb effect. (The pileup is about a 1\% effect, including the pileup errors is about 1\% effect on that, and then time randomizing the errors is less than that.)}
...
\label{Sec:LM}
	...
	Method: Triple coincidence of clusters (\DB{12912}) \\
...
\label{Sec:CBO}
	...
	\begin{enumerate}
		...
		\item{Again because the ratio method divides out the CBO partially, the ratio fit can have a hard time fitting the CBO lifetime. The ratio fit converges to a $\tau_{cbo}$ value with a large error. This is acceptable for a single fit to the calorimeter sum. but doesn't work for individual calorimeter fits, or fit start time scans where less and less data is fit. In the latter $\tau_{cbo}$ is fixed to the value from the single calorimeter sum fit.}
	...
	\begin{figure}[]
	    ...
	    \caption[TrackerCBOModel]{CBO frequency determined with tracker data as a function of time, for station 12 in the 60H dataset. The frequency was modeled as a sum of a constant with a linear part and two exponentials. It rises sharply at early times and slowly over the rest of the fill. Plot created by James Mott, \DB{14208}.}
...
\label{Sec:FinalFitFunction}
	...
	\end{gather}
	All parameters are floating except for the extra terms in $\omega_{cbo}(t)$ (except $\omega_{0}$) as described in Section \ref{Sec:CBO}. The individual calorimeters use the same fit function, with the phase cbo term included:
	\begin{gather}
			f(t) = N_{cbo}(t) \cdot N_{2cbo}(t) \cdot (1 + A \cdot A_{cbo}(t) \cdot \cos(\omega_{a}t + \phi \cdot \phi_{cbo}(t))),
	\end{gather}
	but now with $\tau_{cbo}$ fixed in the fits. In all the fits, \wa is blinded by some amount of ppm in R, as described in \DB{11232}, with a particular string phrase. Finally, the value of the fit function in each time bin is calculated as the integral of the function over the bin, divided by the bin width. It's important to note that fitting with the integral is necessary in order to fit the asymmetry properly, corresponding to the asymmetry value returned by a T Method likelihood fit.
