\graphicspath{ {Figures/TMethod/ResidualsFFT/} {Figures/TMethod/Comparison/} }
...
Finally, the differences in the mean of the R distributions between the T method fit and the ratio fit for each pseudo-experiment is plotted in Figure \ref{Subfig:DiffDist}. It is the width of this histogram that is the number we are after. What this plot tells us is that for a dataset with approximately the same statistics as the 60H dataset, an R value for a T method fit that is 70 ppb different from the R value for a ratio fit, is statistically consistent to 1 $\sigma$. As shown in previous sections, the R values for the T method and ratio method fits of blank and blank are consistent to within blank $\sigma$. Assuming the data is treated in the same way between different analyzers and myself, this should hold true when comparing our results.



Should probably include my T method data randomization results as well here (or earlier in this chapter).
